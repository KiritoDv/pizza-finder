...
class _SavedDataState extends State<SavedDataView> {
  ...
  fetchData() async {
    var data = LocalData.db.fetchItems();
    setState(() {
      this.items = data;
    });
  }

  Widget pizzaItem(PizzaModel item){
    final Size size = MediaQuery.of(context).size;    
    return InkWell(
      onTap: () => ItemManager.showItemPreview(context, fb, item),
      child: Container(
              ...
              child: Padding(
                ...
                child: Container(
                  ...
                  child: InkWell(
                    ...
                    onTap: () async {
                      var pdb = LocalData.db;
                      if(pdb.itemExists(item.id))
                        await pdb.removeItem(item.id);
                      else 
                        await pdb.saveItem(item);
                      setState(() {
                        this.items = pdb.fetchItems();
                      });
                    },
                  ),
                )
            ...
            Align(
              ...
              child: Padding(
                ...
                child: Container(
                  ...
                  child: PriceRange(length: item.price),
                )
              ),
            )
          ],
